...
contract DSCEngine is ReentrancyGuard {
        ...
        nonReentrant
    {
        ...
        emit DSCEngine__CollateralDeposited(msg.sender, _tokenCollateralAddress, _amountCollateral);
    ...
    function _redeemCollateral(address _from, address _to, address _tokenCollateralAddress, uint256 _amountCollateral)
        private
    {
        sCollateralDeposited[_from][_tokenCollateralAddress] -= _amountCollateral;
        emit DSCEngine__CollateralRedeemed(_from, _to, _tokenCollateralAddress, _amountCollateral);
        bool sucess = IERC20(_tokenCollateralAddress).transfer(_to, _amountCollateral);
        if (!sucess) {
            revert DSCEngine__TransferFailed();
        }
    }
    ...
    function getTokenAmountFromUsd(address _token, uint256 _usdAmountInWei) public view returns (uint256) {
        AggregatorV3Interface priceFeed = AggregatorV3Interface(sPriceFeeds[_token]);
